...
Automatically generated on Mon, 30 Mar 2015 23:05:53 -0700
...
int qcsapi_security_add_nai_realm(const char * ifname, int encoding, char * nai_realm, char * eap_method)
{
	int retries = 0;
	int ret;
	CLIENT *clnt = qcsapi_adapter_get_client();
	...
	if (__resp.return_code >= 0) {
		if (nai_realm && __resp.nai_realm)
			strcpy(nai_realm, __resp.nai_realm->data);
	}
	if (__resp.return_code >= 0) {
		if (eap_method && __resp.eap_method)
			strcpy(eap_method, __resp.eap_method->data);
	}
	if (debug) { fprintf(stderr, "%s:%d %s post\n", __FILE__, __LINE__, __FUNCTION__); }
...
int qcsapi_security_add_venue_name(const char * ifname, char * lang_code, char * venue_name)
{
	int retries = 0;
	int ret;
	CLIENT *clnt = qcsapi_adapter_get_client();
	...
	if (__resp.return_code >= 0) {
		if (lang_code && __resp.lang_code)
			strcpy(lang_code, __resp.lang_code->data);
	}
	if (__resp.return_code >= 0) {
		if (venue_name && __resp.venue_name)
			strcpy(venue_name, __resp.venue_name->data);
	}
	if (debug) { fprintf(stderr, "%s:%d %s post\n", __FILE__, __LINE__, __FUNCTION__); }
...
int qcsapi_security_del_venue_name(const char * ifname, char * lang_code, char * venue_name)
{
	int retries = 0;
	int ret;
	CLIENT *clnt = qcsapi_adapter_get_client();
...
int qcsapi_security_add_oper_friendly_name(const char * ifname, char * lang_code, char * oper_friendly_name)
{
	int retries = 0;
	int ret;
	CLIENT *clnt = qcsapi_adapter_get_client();
	...
	if (__resp.return_code >= 0) {
		if (oper_friendly_name && __resp.oper_friendly_name)
			strcpy(oper_friendly_name, __resp.oper_friendly_name->data);
	}
	if (debug) { fprintf(stderr, "%s:%d %s post\n", __FILE__, __LINE__, __FUNCTION__); }
...
int qcsapi_security_del_oper_friendly_name(const char * ifname, char * lang_code, char * oper_friendly_name)
{
	int retries = 0;
	int ret;
	CLIENT *clnt = qcsapi_adapter_get_client();
